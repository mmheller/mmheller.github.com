...
define([
    ...
    return declare([], {
        ...
        addReservoirConditionFeatureLayer: function (arrayOIDYellow, arrayOIDsGold, arrayOIDsOrange, arrayOIDPlum, arrayOIDsRed) {
            ...
            //\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
            console.log("add Stream Condition FeatureLayer and custom legend - Reservoir")
            ...
            let defaultUniqueSymbolRendererCFSRes = {
            ...
            app.pSup.m_StreamStatusRendererCFSRes = defaultUniqueSymbolRendererTemp;
            app.pSup.m_StreamStatusRendererCFSRes.defaultLabel = "Stream Section (Open)";

            app.pSup.m_StreamStatusRendererTempRes = defaultUniqueSymbolRendererCFSRes;
            app.pSup.m_StreamStatusRendererTempRes.defaultLabel = "Stream Section (Open)";
            ...
            if (ArrayUniqueVals2Add.length > 0) {  //getting an error when trying to use addUniqueValueInfo, I think due to the google chart api conflict, so using universal adding to an array then adding to the unique value renderer dictionary
                ...
                app.pSup.m_StreamStatusRendererCFSRes["valueExpression"] = strValueExpression;
                app.pSup.m_StreamStatusRendererCFSRes["uniqueValueInfos"] = ArrayUniqueVals2Add;
            ...
            if (ArrayUniqueVals2AddTemp.length > 0) {  //getting an error when trying to use addUniqueValueInfo, I think due to the google chart api conflict, so using universal adding to an array then adding to the unique value renderer dictionary
                ...
                app.pSup.m_StreamStatusRendererTempRes["valueExpression"] = strValueExpression;
                app.pSup.m_StreamStatusRendererTempRes["uniqueValueInfos"] = ArrayUniqueVals2AddTemp;
            }
            ...
            let featureLayerCFS = new FeatureLayer({
                ...
                outFields: ["OBJECTID"], 
                id:"5555",
                renderer: app.pSup.m_StreamStatusRendererCFSRes
            });
            ...
            let featureLayerTemp = new FeatureLayer({
                ...
                outFields: ["OBJECTID"],
                id: "6666",
                renderer: app.pSup.m_StreamStatusRendererTempRes
            });
            ...
            console.log("Completed: Add Stream Condition FeatureLayer and custom legend - Reservoir 2")
        ...
        addStreamConditionFeatureLayer: function (arrayOIDYellow, arrayOIDsGold, arrayOIDsOrange, arrayOIDPlum, arrayOIDsRed, arrayOIDsGreen, arrayOIDsPurple) {
            ...
            //\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
            console.log("add Stream Condition FeatureLayer and custom legend - Streams")
            let strValueExpression = "";
            let arrayValueExpression = [];
            let arrayValueExpressionTemp = [];
            ...
            if ((arrayOIDsRed.length == 0) & (arrayOIDYellow.length == 0) & (arrayOIDsGold.length == 0) & (arrayOIDsOrange.length == 0) & (arrayOIDsGreen.length == 0) & (arrayOIDsPurple.length == 0)){   ///only add if unique value arrays are blank, otherwise need to specify the "field" in the dictionary to avoid error
                defaultUniqueSymbolRendererCFS["field"] = "OBJECTID";
            }
            ...
            if (arrayOIDPlum.length == 0) {  ///only add if unique value arrays are blank, otherwise need to specify the "field" in the dictionary to avoid error
                defaultUniqueSymbolRendererTemp["field"] = "OBJECTID";
            }
        ...
        Phase1: function () {
            ...
            app.arrayEntireList = [["Beaverhead/Centennial", "Beaverhead", "UMH"],          //array [watershed listed on website, watershed in layer, basin name in website]
                ...
                ["Gallatin-Lower", "Lower Gallatin", "UMH"],
                ["Gallatin-Upper", "Upper Gallatin", "UMH"],
                ["Jefferson", "Jefferson", "UMH"],
                ["Madison", "Madison", "UMH"],
                ["Ruby", "Ruby", "UMH"],
                ...
                ["Flatwillow", "Flatwillow", "Musselshell"],
                ["Lower Musselshell", "Lower Musselshell", "Musselshell"],
                ["Middle Musselshell", "Middle Musselshell", "Musselshell"],
                ["Upper Musselshell", "Upper Musselshell", "Musselshell"],
                
                ["Blackfoot", "Blackfoot", "Blackfoot-Sun"],
                ["Sun", "Sun", "Blackfoot-Sun"],

                ["Little Bighorn", "Little Bighorn", "Bighorn"],
                ["Little Wind", "Little Wind", "Bighorn"],
                ["Lower Wind", "Lower Wind", "Bighorn"],
                ["Lower Bighorn", "Lower Bighorn", "Bighorn"],
                ["North Fork Shoshone", "North Fork Shoshone", "Bighorn"], ["Bighorn Lake", "Bighorn Lake", "Bighorn"],
                ["Shoshone", "Shoshone", "Bighorn"],
                ["South Fork Shoshone", "South Fork Shoshone", "Bighorn"], ["Upper Wind", "Upper Wind", "Bighorn"],
                ["Upper Bighorn", "Upper Bighorn", "Bighorn"],
            ...
            if (!(app.blnPickArea)){
                ...
                if (app.Basin_ID == "Poplar") {
                    $("#dropDownId").append("<li><a data-value='Clear Lake Aquifer Project'>Clear Lake Aquifer Project</a></li>")
                }
            ...
            app.strHFL_URL = "https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest/services/RCT_DRAFT_20251230/FeatureServer/"; //MMH Dev Edit
        ...
        Phase2: function () {
            ...
            app.map = new Map({ basemap: "topo-vector" }); // Basemap layer
            ...
            //const allowedBasemapTitles = ["Imagery Hybrid", "OpenStreetMap Style (with Relief)", "Dark Gray Canvas"];
            //const allowedBasemapTitles = ["Imagery Hybrid", "OpenStreetMap Style", "Dark Gray Canvas"];
            ...
            var pGageFeatureLayer = new FeatureLayer({
            ...
            if (!(app.blnPickArea)) {
                ...
                if (app.StateArea.indexOf("MT") > -1) {
                    ...
                    let strlabelFieldMtMeso11 = "name";
                    const MTMeso_labelClass = {// autocasts as new LabelClass()
                        symbol: { type: "text", color: [112, 112, 112], font: { family: "arial", size: 9 } },
                        labelExpressionInfo: { expression: "$feature." + strlabelFieldMtMeso11 },
                        minScale: 2000000
                    };

                    var templateMTMesoNet = new PopupTemplate();
                    templateMTMesoNet.title = "Montana Mesonet Station";
                    templateMTMesoNet.content = "Station Name:{name} <a href={URL} target='_blank'>(link here)</a></br>Station Abbrev:{station}</b>";

                    var pMTMesoNETFeatureLayer = new FeatureLayer({
                        url: "https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest/services/Montana_Mesonet_Stations/FeatureServer/0",
                        "opacity": 0.9, autoGeneralize: true,
                        id: "MTMesoNet",
            ...
            let pCartoFeatureLayer = new FeatureLayer({
                url: app.strHFL_URL + app.idx11[4],
                id:"Carto", "opacity": 0.9, autoGeneralize: true
            });
            ...
            if (app.Basin_ID == "Poplar") {
                var templateMonitoringWellsClearLake = new PopupTemplate();
                templateMonitoringWellsClearLake.title = "Monitoring Well: " + "{ site_name }";
                templateMonitoringWellsClearLake.content = "<b>{site_name}</b><br><a href={report_link} target='_blank'>Link to report</a><br>Total Depth: {total_depth}";


                var pClearLakeMonitoringWells = new FeatureLayer({
                    url: "https://mbmgmap.mtech.edu/server/rest/services/Water_Resources/GWAP_ClearLakeAquifer/FeatureServer/11",
                    "opacity": 0.5, autoGeneralize: true,
                    id:"clearLakeMonitoringWells",
                    popupTemplate: templateMonitoringWellsClearLake, visible: true
                });

                var pClearLakePermitType = new FeatureLayer({
                    url: "https://mbmgmap.mtech.edu/server/rest/services/Water_Resources/GWAP_ClearLakeAquifer/FeatureServer/3",
                    id:"clearLakePermit","opacity": 0.5, autoGeneralize: true, visible: true
                });
            }
            ...
            if (app.Basin_ID == "Flathead") {
                ...
                var p1964FloodFlathead = new FeatureLayer({
                    ...
                    id:"1964flood", "opacity": 0.5, autoGeneralize: true, visible: false
                });    
            }
            ...
            if (app.Basin_ID == "Poplar") {
                arrayLayer2add.push(pClearLakeMonitoringWells);
                arrayLayer2add.push(pClearLakePermitType);
            }
            ...
            if (app.Basin_ID == "UMH") {
                ...
                pCartoFeatureLayerPoly.title = 'Special Areas of Interest';
                pCartoFeatureLayerPoly.definitionExpression = "WatershedName <> 'Crown of the Continent'";
            }
            ...
            if ((app.Basin_ID == "Crown of the Continent - NE") | (app.Basin_ID == "Crown of the Continent - NW") |
                ...
                pCartoFeatureLayerPoly.title = 'Crown of the Continent';
                pCartoFeatureLayerPoly.definitionExpression = "WatershedName = 'Crown of the Continent'";
            }
            ...
            if (app.Basin_ID == "Poplar") {
                legendLayers.push({ layer: pClearLakeMonitoringWells, title: 'ClearLake Aquifer Project: Monitoring Wells' });
                legendLayers.push({ layer: pClearLakePermitType, title: 'ClearLake Aquifer Project: Permit Types' });
            }
            ...
            if ((app.Basin_ID == "Crown of the Continent - NE") | (app.Basin_ID == "Crown of the Continent - NW") |
                (app.Basin_ID == "Crown of the Continent - SE") | (app.Basin_ID == "Crown of the Continent - SW") |
                (app.Basin_ID == "UMH") |
                (app.Basin_ID == "Flathead")) {
                pCartoFeatureLayerPoly.visible = true;
                cbxLayers.push({ layers: [pCartoFeatureLayerPoly, pCartoFeatureLayerPoly], title: pCartoFeatureLayerPoly.title });
            }
            ...
            if (app.Basin_ID == "Poplar") {
                cbxLayers.push({ layers: [pClearLakeMonitoringWells, pClearLakeMonitoringWells], title: 'ClearLake Aquifer Project: Monitoring Wells' });
                cbxLayers.push({ layers: [pClearLakePermitType, pClearLakePermitType], title: 'ClearLake Aquifer Project: Permit Types' });
            }
            ...
            function SetupJump2Map() {
                $('#dropDownId a').click(function () {
                    ...
                    if (strSelectedText == "Clear Lake Aquifer Project") {
                        blnAddCoords = false;
                        pSR_WKID = pExtent.spatialReference.wkid;
                        strURL = "https://gis-data-hub-mbmg.hub.arcgis.com/apps/e6c4ab546b5e49eb894e3ff5b18ac52f/explore";
