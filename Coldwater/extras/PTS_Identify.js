  ...
  declare, lang, esriRequest, all, arrayUtils, SimpleMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol, Color, CK_AutoCompleteSearchAndZoom
    ...
    return declare([], {
        ...
        showFeature: function (pFeature, strTheme) {
            ...
            var strTitle = attr.FishID;
            strTitle = strTitle.split(/((?:\w+ ){7})/g).filter(Boolean).join("<br>");

            var tempstrcontent = ""
            tempstrcontent += "<u>FishID:</u>  " +attr.FishID + "<br />" +
                                     "<u>Date/Time:</u>  " + formatDateTime(attr.Date_Time_noMin) + "<br />" +
                                     "<u>Receiver:</u>  " + attr.Receiver + "<br />" +
                                     "<u>TagIDnum:</u> " + attr.TagIDnum + "<br />" +
                                     "<u>Temp (c):</u> " + attr.Temp_c + "<br />" +
                                     "<u>Depth_m:</u>  " + attr.Depth_m + "<br />" +
                                     "<u>OBJECTID:</u>  " + attr.OBJECTID + "<br />"
            this.strFeatureContent = tempstrcontent;
            //this.strFeatureContent += "(<A href='#' onclick='showReportPage(" + attr.FishID + ");'>Show project detail</A>)<br/>";

            this.pMap.infoWindow.setContent(this.strFeatureContent);
            this.pMap.infoWindow.setTitle(strTitle);
            this.pMap.infoWindow.resize(315, 300);
...
//            this.mTc.resize({ w: 400, h: 235 });
        ...
        showFeatureSet: function (results) {
            ...
            var content = "<u>Number of detections:  " + pLayer1.length + "</u><br />";
            arrayUtils.forEach(pLayer1, function (feat) {
                feat.setSymbol(new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID, new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_SQUARE, 10,
                                                                                                            new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,new Color([255, 0, 0]), 1),
                                                                                                    new Color([0, 255, 0, 0.25])))); app.map.graphics.add(feat) ;
        }); // add the results to the map
            for (var iii = 0; iii < pLayer1.length; iii++) {
                var graphic3 = pLayer1[iii];
                strThemeT = "Point";
                strURL4query3 = this.strURL + "0";
                if (graphic3.attributes.FishID != null) {
                    var strTrimmedTitle = graphic3.attributes.FishID + ":" + formatDateTime(graphic3.attributes.Date_Time_noMin);
                } else {
                    var strTrimmedTitle = "unknown title";
                }
                content += "  " + strTrimmedTitle + " (<A href='#' onclick='showFeaturePrep(pLayer1[" + iii + "],strURL4query3,strThemeT);'>show</A>)<br/>";
                //content += "  " + strTrimmedTitle + " (" + graphic3.attributes.OBJECTID + ") (<A href='#' onclick='showFeaturePrep(pLayer1[" + iii + "],strURL4query3,strThemeT);'>show</A>)<br/>";
            }
        ...
        returnEvents: function (results) {
            ...
            if (this.m_dblY != null) {
                var CK_ASZ = new CK_AutoCompleteSearchAndZoom({ pMap: app.map, dblExpandNum: 2,
                    ...
                    strSearchField: "OBJECTID", pSR: this.mSR
